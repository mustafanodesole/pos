      ...
      >
        ...
        <template v-slot:body-cell-profit="props">
          <Td class="text-center">
            <span :class="props.row.profit < 0 ? 'text-red' : ''">
              Rs. {{ props.row.profit }} /-
            </span>
          </Td>
        </template>
          ...
          <Td>
            <div class="row items-center justify-center">
              ...
              <q-btn
                ...
                :color="$q.dark.isActive ? 'white' : 'black'"
                @click.stop="handleSellItems(props.row)"
                class="col-1"
              />
              ...
              <q-btn
                ...
                icon="sell"
                :color="$q.dark.isActive ? 'white' : 'black'"
                @click.stop="toggleSellInput(props.row)"
                class="col-6"
              >
                <q-tooltip
                  class="bg-black"
        ...
        <q-card style="min-width: 350px">
          <q-card-section class="flex justify-between">
            <div class="text-h6 text-capitalize">
              {{ selectedRow?.product_name || "Product Name" }}
            </div>
    ...
    .then((result) => {
      ...
      result.data.forEach((item) => {
        rows.value.push({
          ...
          profit: item.retail_price - item.purchased_price,
          remaining: item.total_items + " Items",
        });
      });
    });
}
...
let filterData = computed(() => {
  ...
  return rows.value.filter(
    (item) =>
      ...
      item.retail_price.includes(searchProduct.value.toLocaleLowerCase())
  );
});
filterData;
...
const handleUpdatePrice = async () => {
  ...
  try {
    ...
    selectedRow.value.remaining = `${totalItems.value} Items`;
